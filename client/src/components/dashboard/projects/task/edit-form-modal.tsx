...
import { updateTaskById } from "@/fetch-api/task";
...
const EditFormModal = ({ state, onClose }: EditFormModalProps) => {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      status: state?.status,
      assigns: state?.assigns || undefined,
      attachments: state.attachments as IAttachment[],
      dueDate: state?.dueDate ? new Date(state?.dueDate) : undefined,
      priority: state?.priority,
      estimateHours: state?.estimateHours || undefined,
    });
  }, [state, reset]);
  ...
  const onSubmit: SubmitHandler<TaskInput> = async (data) => {
    try {
      setIsLoading(true);
      const body = { ...data, assigns: data?.assigns?.map((data) => data?._id) };
      if (!(data.attachments instanceof File)) delete body.attachments;
      const formData = initializeFormData(body);

      const { error, message, success } = await updateTaskById(state?._id, formData);
      if (success) {
        closeModal();
            ...
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Field data-invalid={fieldState.invalid} className="gap-1 col-span-2">
                  ...
                  <SelectMembers defaultUsers={field.value} onChange={field?.onChange} />
                  {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                </Field>
              )}
            />
